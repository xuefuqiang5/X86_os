...
struct task_struct* thread_start(char* name, int prio, thread_func function, void* func_arg){
    ...
    assert(!list_find(&ready_head, &thread->general_tag));
    list_pushback(&thread->general_tag, &ready_head);
...
void schedule(){
    ...
    if(cur->status == TASK_RUNNING){
        assert(!list_find(&ready_head, &cur->general_tag));
        list_pushback(&cur->general_tag, &ready_head);
        cur->ticks = cur->priority;
        cur->status = TASK_READY;
    }
    else {}
    struct task_struct* next = to_entry(list_pop(&ready_head), struct task_struct, general_tag); 
